## from https://github.com/kahst/BirdNET/
...
# DVM:
def spectrogram224(sig, rate, shape=(224, 224), win_len=512, fmin=150, fmax=15000, frequency_scale='mel', magnitude_scale='nonlinear', bandpass=True, decompose=False):
	...
	# Define mel filter bank 
        n_mels = shape[1]
        mel_basis = librosa.filters.mel(sr=rate, n_fft=n_fft, n_mels=n_mels, fmin=fmin, fmax=fmax)
        spec = np.dot(spec, mel_basis)
        
        '''import matplotlib.pyplot as plt
        plt.plot(mel_basis)
        plt.show()
        plt.savefig('mel-filter-bank.png')
        sys.exit(0)'''
